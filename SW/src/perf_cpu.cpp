...
void perf_cpu::status_log(uint32_t *perf_array) 
{
    ...
    LOG("    Cycle_cnt: " << l_cycle_cnt);
    LOG("    Instruction_cnt: " << l_inst_cnt);
    LOG("    CPI: " << l_cpi);
    LOG("    Total NOPs: " << l_tot_nop);
    LOG("    HW NOPs: " << l_hw_nop);
    LOG("    Compiler NOPs: " << l_tot_nop - l_hw_nop);
    LOG("");
    LOG("    Dependency detection:");
    LOG("        On RS1 in EX stage: " << l_dd_rs1_ex_cnt);
    LOG("        On RS2 in EX stage: " << l_dd_rs2_ex_cnt);
    LOG("        On RS1 in MEM stage: " << l_dd_rs1_mem_cnt);
    LOG("        On RS2 in MEM stage: " << l_dd_rs2_mem_cnt);
}
