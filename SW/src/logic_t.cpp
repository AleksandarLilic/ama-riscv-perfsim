...
// Methods
uint32_t logic_t::get() { return m_value; }
void logic_t::rst() { m_temp_value = m_rst_value; }  // sync rst, needs active edge to take rst val
void logic_t::set_name(std::string init_n) { m_name = init_n; }
...
uint32_t logic_t::operator+ (const uint32_t val_in) { return (get() + val_in); }
uint32_t logic_t::operator- (const uint32_t val_in) { return (get() - val_in); }
void logic_t::operator+= (const uint32_t val_in) { m_temp_value = get() + val_in; }
void logic_t::operator-= (const uint32_t val_in) { m_temp_value = get() - val_in; }
...
{
    uint32_t res;
    res = get() + logic_in.m_value;
    return res;
}
logic_t logic_t::operator- (const logic_t logic_in)
{
    logic_t res(0);
    res.m_temp_value = get() - logic_in.m_value;
    return res;
}
